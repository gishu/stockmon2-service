...
function descPriceComparer(left, right) {
  return right.price.comparedTo(left.price);
}
function ascPriceComparer(left, right) {
  return left.price.comparedTo(right.price);
}
function orderByDate(left, right) {
  if (left.date.isSame(right.date, "day")) 
    return 0;

  return left.date.isBefore(right.date, "day") ? -1 : 1;
}
...
module.exports = function(buys, sales) {
  ...
  function matchSale(sale) {
    ...
    if (saleQty > 0) {
      throw new Error(
        util.format(
          "Insufficient holdings to sell %d of %s!",
          sale.qty,
          sale.stock
        )
      );
  ...
  function getSplice() {
    ...
    for (looper = 0; looper < salesQueue.length; looper++) {
      ...
      totalSold += sale.qty;
      var totalAvailable = _.reduce(
        getBuysBefore(sale.date),
        (totalAvailable, buy) => totalAvailable + buy.balance,
        0
      );
